...
class TreeList {
    ...
    */
    ...
    public static Node toList(Node root) {
        ...
        Node previous = toList(root.small);
        Node next = toList(root.large);
        if (previous == null && next == null) return root;

        if (previous != null) {
            while(previous.large != null)previous=previous.large;
            // previous.small = next;
            previous.large = root;
            root.small = previous;
        }
        if (next != null) {
            while(next.small != null)next=next.small;
            // next.large = previous;
            root.large = next;
            next.small = root;
        }
        while(root.small != null)root=root.small;
        return root;
    }
    ...
    public static void printmyList(Node head) {
        Node curr = head;
        while (curr != null) {
            System.out.print(curr.data+",");
            curr = curr.large;
            // if (curr.data == tail.data)break;
        }
    }
